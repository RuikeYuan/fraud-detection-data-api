...
TOOLS = [
    {
        ...
        "description": "Get current transaction graph statistics: node count, edge count, high-risk account counts, model info.",
        "input_schema": {
            "type": "object",
            "properties": {},
            "required": [],
    ...
    },
    {
        ...
        "description": (
            "Evaluate a transaction's fraud risk using the GraphSAGE GNN model. "
            "Returns sender and receiver fraud probabilities, combined risk level (HIGH/MEDIUM/LOW), and rationale."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "src":     {"type": "string", "description": "Sender account ID (nameOrig)"},
                "dst":     {"type": "string", "description": "Receiver account ID (nameDest)"},
    ...
    },
    {
        ...
        "description": "Retrieve historical transaction profile for an account: transaction frequency, cumulative amounts, GNN fraud probability.",
        "input_schema": {
            "type": "object",
            "properties": {
...
def run_fraud_investigation(tx: dict) -> str:
    ...
    system_prompt = """You are a financial fraud investigation AI Agent specializing in suspicious transaction analysis.
You have access to the following tools:
- get_graph_status: retrieve current transaction graph statistics
- score_transaction: evaluate transaction fraud risk using the GraphSAGE GNN model
- get_account_profile: retrieve account history and GNN fraud probability

Investigation workflow:
1. Call score_transaction to obtain GNN risk scores
2. Call get_account_profile for both sender and receiver
3. Synthesize all findings into a structured investigation report

Report format (all sections required, in English):
## Transaction Overview
| Field | Value |
(table with Transaction ID, Type, Sender, Receiver, Amount, Time Step)

## GNN Model Scores
| Metric | Result |
(table with sender prob, receiver prob, overall risk level, model name, rationale)

## Account Profiles
### Sender: <id>
### Receiver: <id>
(bullet points with GNN prob, risk indicators, behavioral notes)

## Verdict
> **Suspected Fraud / Legitimate / Inconclusive** | Confidence: High/Medium/Low
(2-3 sentence summary of key risk signals)
...
| FOLLOW-UP | ... |"""
    ...
    user_message = (
        f"Investigate the following transaction for fraud risk:\n\n"
